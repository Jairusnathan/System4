...
export function parseSerializedAddresses(
  ...
  if (address.startsWith(ADDRESS_STORAGE_PREFIX)) {
    ...
    } catch (error) { console.error('Failed to parse saved addresses:', error); }
  }
  return [{ fullName: user?.full_name || '', phoneNumber: user?.phone || '', province: '', city: '', postalCode: '', streetAddress: address, label: 'Home' }];
}
...
export function normalizeSavedAddresses(
  ...
  return addresses.map((entry) => {
    ...
    return { ...entry, fullName: entry.fullName || fallback?.full_name || '', phoneNumber: normalizedPhone || '', label };
  });
}
