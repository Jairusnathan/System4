...
export class AuthController {
  ...
  /** Returns the admin client, or null when SUPABASE_SERVICE_ROLE_KEY is not set. */
  private tryGetAdmin() {
    try {
  ...
  async login(
    ...
    try {
      ...
      // Check account lockout (requires failed_login_attempts + account_locked_until columns on customers)
      ...
      if (!isPasswordValid) {
        ...
        if (shouldLock) {
          updateData.account_locked_until = new Date(Date.now() + 60 * 60 * 1000).toISOString();
        }
        ...
        void (async () => {
          try {
            const { error: updateErr } = await this.supabaseService.supabase
              ...
              .update(updateData)
              .eq('id', user.id);
            if (!updateErr && shouldLock && this.mailerService.isConfigured()) {
              await this.mailerService.sendAccountLockedEmail(email, user.full_name || 'User');
            }
          } catch { /* ignore — never block login error response */ }
        })();
      ...
      if (admin) {
        try {
          ...
          await admin.from('refresh_token_families').insert({
            user_id: user.id,
            token_hash: tokenHash,
            family_id: crypto.randomUUID(),
            expires_at: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
          });
        } catch { /* non-fatal — token rotation unavailable */ }
      }
  ...
  async refresh(
    ...
    if (admin) {
      ...
      const { data: storedToken } = await admin
        ...
        .select('id, family_id, revoked, user_id')
        .eq('token_hash', tokenHash)
        .single();

      if (storedToken?.revoked) {
        console.warn(`[auth] Refresh token reuse detected for user ${storedToken.user_id} — revoking all sessions`);
        await admin
          .from('refresh_token_families')
          .update({ revoked: true })
      ...
      const nextHash = Buffer.from(nextRefreshToken).toString('base64url').slice(0, 64);
      const familyId = storedToken?.family_id ?? crypto.randomUUID();
    ...
    // No admin client — skip token-family tracking, just re-sign
    const nextAccessToken = this.authService.signAccessToken(payload);
    const nextRefreshToken = this.authService.signRefreshToken(payload);
    this.authService.setRefreshTokenCookie(response, nextRefreshToken);
    return { token: nextAccessToken };
  ...
  async updateProfile(
    ...
    try {
      const userId = this.authService.requireUserId(authorization);
      const { full_name, phone, birthday, gender, address, profile_image } =
        body;
      ...
      let normalizedAddresses: ReturnType<
        typeof normalizeSavedAddresses
      > | null = null;
