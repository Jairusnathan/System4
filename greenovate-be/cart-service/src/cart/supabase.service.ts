...
export class SupabaseService implements OnModuleInit {
  ...
  private initializeDefaultClient(): void {
    const url = this.configService.get<string>('OOS_CART_SUPABASE_URL')?.trim();
    const key = this.configService.get<string>('OOS_CART_SUPABASE_SERVICE_ROLE_KEY')?.trim();
    const anonKey = this.configService.get<string>('OOS_CART_SUPABASE_ANON_KEY')?.trim();
    if (url && anonKey) this.publicClient = this.buildClient(url, anonKey, false);
    if (!url || !key) { this.logger.warn('OOS_CART_SUPABASE_URL or OOS_CART_SUPABASE_SERVICE_ROLE_KEY is not set.'); return; }
    this.client = this.buildClient(url, key);
  }

  private initializeLegacyClients(): void {
    const secondUrl = this.configService.get<string>('OOS_CART_SECOND_SUPABASE_URL')?.trim();
    const secondAnonKey = this.configService.get<string>('OOS_CART_SECOND_SUPABASE_ANON_KEY')?.trim();
    const secondServiceRoleKey = this.configService.get<string>('OOS_CART_SECOND_SUPABASE_SERVICE_ROLE_KEY')?.trim() ?? secondAnonKey;
