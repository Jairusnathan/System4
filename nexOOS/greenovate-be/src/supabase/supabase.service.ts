...
export class SupabaseService implements OnModuleInit {
  ...
  get secondSupabase(): SupabaseClient {
    return this.ensureClient(
      this.secondClient,
      'SECOND_SUPABASE_URL + SECOND_SUPABASE_ANON_KEY',
    );
  }

  get secondSupabaseAdmin(): SupabaseClient {
    return this.ensureClient(
      this.secondAdminClient,
  ...
  private initializeDefaultClient(): void {
    ...
    const anonKey =
      ...
      this.configService.get<string>('NEXT_PUBLIC_SUPABASE_ANON_KEY')?.trim();
    const publicUrl =
      url ?? this.configService.get<string>('NEXT_PUBLIC_SUPABASE_URL')?.trim();

    if (publicUrl && anonKey) {
      this.publicClient = this.buildClient(publicUrl, anonKey, false);
    }
  ...
  private initializeLegacyClients(): void {
    const secondUrl =
      this.configService.get<string>('SECOND_SUPABASE_URL')?.trim() ??
      this.configService.get<string>('NEXT_PUBLIC_SECOND_SUPABASE_URL')?.trim();
    const secondAnonKey =
      this.configService.get<string>('SECOND_SUPABASE_ANON_KEY')?.trim() ??
      this.configService
        .get<string>('NEXT_PUBLIC_SECOND_SUPABASE_ANON_KEY')
        ?.trim();
    const secondServiceRoleKey =
      this.configService
        .get<string>('SECOND_SUPABASE_SERVICE_ROLE_KEY')
        ?.trim() ?? secondAnonKey;

    if (secondUrl && secondAnonKey) {
      this.secondClient = this.buildClient(secondUrl, secondAnonKey, false);
    }

    if (secondUrl && secondServiceRoleKey) {
      this.secondAdminClient = this.buildClient(
        secondUrl,
        secondServiceRoleKey,
      );
    }
  }
  ...
  private ensureClient(
    ...
    throw new Error(
      `Supabase client '${configName}' is not configured for this environment.`,
    );
  }
